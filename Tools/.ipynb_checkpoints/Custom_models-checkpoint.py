...
class Model_1D(tf.keras.Model):
    ...
    def __init__(self, N_nodes, N_output=1, reg_kernel=False, reg_biases=False, reg_value=0.01, name=None):
...
class Model_1D_1_layer(tf.keras.Model):
    ...
    def __init__(self, N_nodes, N_output=1, name=None):
        super(Model_1D_1_layer, self).__init__(name=name)
        self.dense1 = Dense(N_nodes, activation='relu', name='dense1')
        self.dense2 = Dense(N_output, activation=None, name='dense2')
